...
public class PersonResourceTest {
  final Logger logger = LoggerFactory.getLogger(PersonResourceTest.class);
  ...
  public void testBookFind() {
    ...
    // when
    final String messagePersons = client.target(
        String.format("http://localhost:%d/persons", RULE.getLocalPort()))
        .request()
        .get(String.class);

    logger.info("Persons: " + messagePersons);

    final JSONArray jsonPersons = getJSONArray(messagePersons);
    assertTrue(jsonPersons.length() > 0);

    final JSONObject person = jsonPersons.getJSONObject(0);

    // when
    final String message = client.target(
        String.format("http://localhost:%d/persons/" + person.get("id"), RULE.getLocalPort()))
        .request()
        .get(String.class);
    ...
    // then
    assertThat(json.getString("name")).isEqualTo(person.getString("name"));
  }
